shared_examples :increment do
  it 'initializes in #increment with 1' do
    ...
    expect(store.raw['inckey']).to match(/^1\b/)
    expect(store.raw.load('inckey')).to match(/^1\b/)
    expect(store.load('inckey', raw: true)).to match(/^1\b/)

    expect(store.delete('inckey', raw: true)).to match(/^1\b/)
    expect(store.key?('inckey')).to be false
  end
  ...
  it 'initializes in #increment with higher value' do
    ...
    expect(store.key?('inckey')).to be true
    expect(store.raw['inckey']).to match(/^42\b/)
    expect(store.delete('inckey', raw: true)).to match(/^42\b/)
  end

  it 'initializes in #increment with 0' do
    expect(store.increment('inckey', 0)).to eq 0
    expect(store.key?('inckey')).to be true
    expect(store.raw['inckey']).to match(/^0\b/)
    expect(store.delete('inckey', raw: true)).to match(/^0\b/)
  end
  ...
  it 'supports incrementing existing value by value' do
    ...
    expect(store.raw['inckey']).to match(/^43\b/)
  end
  ...
  it 'supports decrementing existing value by value' do
    ...
    expect(store.raw['inckey']).to match(/^40\b/)
  end
  ...
  it 'supports decrementing existing value' do
    ...
    expect(store.raw['inckey']).to match(/^5\b/)
