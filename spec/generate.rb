...
SIMPLE_SPECS = [:null, :store, :returndifferent, :marshallable_key, :marshallable_value, :transform_value]
NATIVE_EXPIRES_SPECS = SIMPLE_SPECS + [:expires_stringkey_stringvalue]
EXPIRES_SPECS = NATIVE_EXPIRES_SPECS + [:transform_value_with_expires]
EXPIRES_SPECS.delete(:transform_value)
...
it 'should support multiple clients' do
  ...
  'simple_memory_with_json_value_serializer' => {
    ...
    :value => %w(Hash String),
    :specs => [:null, :store, :returndifferent, :marshallable_key, :transform_value],
    :load_value => '::MultiJson.load(value).first'
  },
  'simple_memory_with_quicklz_compress' => {
    :store => :Memory,
    :options => ':compress => :quicklz',
    :load_value => 'Marshal.load(::QuickLZ.decompress(value))'
  },
  'simple_lruhash' => {
    :store => :LRUHash,
  },
