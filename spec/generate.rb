...
SIMPLE_SPECS = [:null, :store, :returndifferent, :require_marshallable]
...
TESTS = {
  ...
  'simple_null' => {
    :store => :Null,
    :specs => [:null, :require_marshallable, :returndifferent]
  },
  'null_adapter' => {
  ...
  adapter :File, :dir => File.join(make_tempdir, "expires-file")
    ...
    :specs => [:null, :store, :expires, :returndifferent, :require_marshallable],
    :tests => %{
...
SPECS['require_marshallable']  = %{it 'refuses to #[] from keys that cannot be marshalled' do
  expect do
