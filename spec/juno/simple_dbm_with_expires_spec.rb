...
describe_juno "simple_dbm_with_expires" do
  def new_store
    Juno.new(:DBM, :file => File.join(make_tempdir, "simple_dbm_with_expires"), :expires => true, :logger => {:out => File.open(File.join(make_tempdir, 'simple_dbm_with_expires.log'), 'a')})
  end
