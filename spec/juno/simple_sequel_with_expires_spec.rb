...
describe_juno "simple_sequel_with_expires" do
  def new_store
    Juno.new(:Sequel, :db => (defined?(JRUBY_VERSION) ? "jdbc:sqlite:" : "sqlite:") + File.join(make_tempdir, "simple_sequel_with_expires"), :expires => true, :logger => {:out => File.open(File.join(make_tempdir, 'simple_sequel_with_expires.log'), 'a')})
  end
