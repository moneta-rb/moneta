...
describe_moneta "adapter_mongo_moped" do
  ...
  it 'automatically deletes expired document' do
    ...
    store.instance_variable_get(:@collection).find('_id' => ::BSON::Binary.new('key')).one.should_not be_nil
    sleep 70 # Mongo needs up to 60 seconds
    store.instance_variable_get(:@collection).find('_id' => ::BSON::Binary.new('key')).one.should be_nil
  end
end
