...
describe_moneta "simple_activerecord" do
  ...
  def new_store
    Moneta.new(:ActiveRecord, :connection => { :adapter => (defined?(JRUBY_VERSION) ? 'jdbcmysql' : 'mysql2'), :database => 'simple_activerecord' }, :logger => {:file => File.join(make_tempdir, 'simple_activerecord.log')})
  end
