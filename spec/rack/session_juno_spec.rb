...
describe Rack::Session::Juno do
  session_key = Rack::Session::Juno::DEFAULT_OPTIONS[:key]
  ...
  it 'supports different constructors' do
    Rack::Session::Juno.new(incrementor, :store => :Memory)
    Rack::Session::Juno.new(incrementor, :store => Juno.new(:Memory, :expires => true))
    Rack::Session::Juno.new(incrementor) do
      use :Expires
      adapter :Memory
    end
  end
  ...
  it "creates a new cookie" do
    pool = Rack::Session::Juno.new(incrementor, :store => :Memory)
  ...
  it "determines session from params" do
    pool = Rack::Session::Juno.new(incrementor, :cookie_only => false, :store => :Memory)
    req = Rack::MockRequest.new(pool)
    res = req.get("/")
    sid = res["Set-Cookie"][session_match, 1]
    req.get("/?rack.session=#{sid}").
  ...
  it "maintains freshness" do
    pool = Rack::Session::Juno.new(incrementor, :expire_after => 3, :store => :Memory)
    res = Rack::MockRequest.new(pool).get('/')
  ...
  it "updates deep hashes correctly" do
    ...
    pool = Rack::Session::Juno.new(hash_check, :store => :Memory)
    req = Rack::MockRequest.new(pool)
